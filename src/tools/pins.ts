    ...
    async ({ board_id, section_id, page_size, bookmark }) => {
      try {
        ...
        if (result.items.length === 0) {
          return {
            content: [{ type: "text" as const, text: "No pins found." }],
          };
        }
        ...
        return {
          content: [{ type: "text" as const, text: `Error listing pins: ${msg}` }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ pin_id }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: formatPinDetails(pin) }],
        };
      } catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
        return {
          content: [{ type: "text" as const, text: `Error getting pin: ${msg}` }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ pin_id, size }) => {
      try {
        ...
        if (!pin.media?.images) {
          return {
            content: [{ type: "text" as const, text: "This pin has no image media." }],
            isError: true,
          };
        }
        ...
        if (!imageInfo) {
          return {
            content: [{
              type: "text" as const,
              text: `No image available in size "${selectedSize}". Available sizes: ${Object.keys(pin.media.images).join(", ")}`,
            }],
            isError: true,
          };
        }
        ...
        const contextText = [
          ...
          `Current description: ${pin.description ?? "(none)"}`,
          `Current alt_text: ${pin.alt_text ?? "(none)"}`,
          `Board ID: ${pin.board_id}`,
          `Section ID: ${pin.board_section_id ?? "(none)"}`,
          `Link: ${pin.link ?? "(none)"}`,
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
          content: [{ type: "text" as const, text: `Error fetching pin image: ${msg}` }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ pin_id, title, description, alt_text, link, board_id, board_section_id }) => {
      try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
          content: [{ type: "text" as const, text: `Error updating pin: ${msg}` }],
          isError: true,
        };
      }
    },
    ...
    async ({ pin_id, board_id, section_id }) => {
      try {
        ...
        return {
          content: [{
            type: "text" as const,
            text: `Pin moved successfully to board ${pin.board_id}${pin.board_section_id ? ` / section ${pin.board_section_id}` : ""}.`,
          }],
        };
      } catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
        return {
          content: [{ type: "text" as const, text: `Error moving pin: ${msg}` }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ board_id, image_url, title, description, alt_text, link, board_section_id }) => {
      try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
          content: [{ type: "text" as const, text: `Error creating pin: ${msg}` }],
          isError: true,
        };
      }
    },
